...
def create_camera(camera: CameraCreate):
    ...
    try:
        # Comprovem que l'usuari propietari existeix
        cur.execute("""
            SELECT id
            FROM users
            WHERE id = %s
        ...
        # Inserim la càmera a la base de dades
        cur.execute("""
            INSERT INTO camera (url, owner_id, latitude, longitude)
            VALUES (%s, %s, %s, %s)
...
def get_camera_stream(camera_id: int, request: Request):
    ...
    - Retorna la URL del Media Server i el token temporal
    """
